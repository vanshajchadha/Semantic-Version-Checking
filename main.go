			...
			if ok == true {
				...
				if releases[i].Patch > elem.Patch {
					m[s] = *releases[i]
					 ...
					 /*if semver.preReleaseCompare(releases[i],elem) == 1 {	
					 	m[s] = *releases[i]  // No need to implement this as we are asked to fetch simply according to the highest patch version in the question
					 }*/						
				}
